...
export class SpaceForceSimulation implements SimState {
  // ---- Entity collections ----
  ...
  private resolveCollisionTelemetryEnabled(): boolean {
    const globalValue = (
      globalThis as { __SPACE_FORCE_COLLISION_TELEMETRY__?: unknown }
    ).__SPACE_FORCE_COLLISION_TELEMETRY__;
    ...
    ).process?.env?.SPACE_FORCE_COLLISION_TELEMETRY;
