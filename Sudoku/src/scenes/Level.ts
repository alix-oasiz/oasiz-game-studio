...
export default class Level extends Phaser.Scene {
    ...
    private levelDefinition: SudokuLevelDefinition = getSudokuLevel(1);
    private levelBestScore: number | null = loadLevelBestScore(1);
    private currentClueCount = this.levelDefinition.clueCount;
    ...
    init(data: LevelSceneData = {}): void {
        ...
        const requestedLevel = typeof data.level === "number" && Number.isFinite(data.level)
            ? Math.trunc(data.level)
            : 1;

        this.levelNumber = Phaser.Math.Clamp(requestedLevel, 1, TOTAL_LEVELS);
        this.levelDefinition = getSudokuLevel(this.levelNumber);
        this.levelBestScore = loadLevelBestScore(this.levelNumber);
        this.currentClueCount = this.levelDefinition.clueCount;
    }
    ...
    private createBoardState(): void {
        ...
        const { solution, givens } = this.generatePuzzle(this.levelDefinition.clueCount);
        this.board = [];
        this.currentClueCount = givens.flat().filter(Boolean).length;
    ...
    private createHeader(): void {
        ...
        this.headerLevelText = this.add.text(GAME_WIDTH / 2, 14, "", {
            fontFamily: FONT_FAMILY,
            fontSize: "15px",
            color: "#dfe5ec",
            fontStyle: "600"
    ...
    private handleWinOverlayAction(action: WinOverlayAction): void {
        ...
        if (action === "main") {
            this.scene.start("MainMenu", { level: this.levelNumber });
            return;
        }

        const nextLevelNumber = getNextLevelNumber(this.levelNumber);
        if (nextLevelNumber !== null) {
            this.scene.start("Level", { level: nextLevelNumber });
            return;
        }
    ...
    private submitFinalScore(): void {
        ...
        if (this.mode === "won") {
            const previousBest = this.levelBestScore;
            this.levelBestScore = saveLevelBestScore(this.levelNumber, score);
            this.newBestScore = previousBest === null || score > previousBest;
        }
    ...
    private calculateFinalScore(): number {
        ...
        const solvedBonus = this.mode === "won" ? this.levelDefinition.completionBonus : 0;
        return Math.max(0, this.score + solvedBonus - this.mistakes * 90 - this.hintsUsed * 60 - seconds * 2);
    }
    ...
    private refreshAll(): void {
        ...
        this.headerTimerText.setText(this.formatTime(Math.floor(this.elapsedMs / 1000)));
        this.headerLevelText.setText(
            `Level ${this.levelNumber} • ${this.levelDefinition.title} • ${this.getBestScoreLabel()}`
        );
    ...
    private refreshOverlay(): void {
        ...
        this.overlayTitle.setText(`Level ${this.levelNumber} Failed`);
        this.overlayBody.setText(`You hit the mistake limit on Level ${this.levelNumber}.\nTap restart to try this board again.`);
        ...
        if (won) {
            ...
            const nextLevelNumber = getNextLevelNumber(this.levelNumber);
            const bestLine = this.levelBestScore === null
                ? "First clear on this level"
                : `Best score ${this.formatScore(this.levelBestScore)}${this.newBestScore ? " • New best" : ""}`;

            this.winOverlayTitle.setText(`LEVEL ${this.levelNumber} CLEAR`);
            this.winOverlayStats.setText(
                `Solved in ${this.formatTime(totalSeconds)}\nScore ${this.formatScore(this.finalScore)}  •  ${bestLine}\n${this.currentClueCount} clues  •  ${mistakeLine}  •  ${hintLine}`
            );
            this.winOverlayMainButtonSubLabel.setText("Back to the level select");
            this.winOverlayNewButtonLabel.setText(nextLevelNumber === null ? "Replay Level" : `Next Level`);
            this.winOverlayNewButtonSubLabel.setText(
                nextLevelNumber === null
                    ? "Roll a fresh Level 10 puzzle"
                    : `Jump into Level ${nextLevelNumber}`
            );
    ...
    private renderGameToText(): string {
        ...
        const payload = {
            ...
            level: {
                ...
                title: this.levelDefinition.title,
                clueCount: this.currentClueCount,
                bestScore: this.levelBestScore
            },
            mode: this.mode,
            timer: this.formatTime(Math.floor(this.elapsedMs / 1000)),
            score: this.score,
            finalScore: this.mode === "playing" ? null : this.finalScore,
    ...
    private getBestScoreLabel(): string {
        ...
        return this.levelBestScore === null
            ? "Best --"
            : `Best ${this.formatScore(this.levelBestScore)}`;
    }
    ...
    private generatePuzzle(targetClues: number): { solution: number[][]; givens: boolean[][] } {
        ...
        const normalizedTargetClues = Phaser.Math.Clamp(Math.round(targetClues), 26, 52);

        for (let index = 0; index < normalizedTargetClues; index++) {
